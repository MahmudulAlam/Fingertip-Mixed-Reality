...
""" Hand Detection """
options = {'model': 'cfg/tiny-yolo-voc-1c.cfg', 'load': 3250, 'threshold': 0.2, 'gpu': 1.0}
tfnet = TFNet(options)

""" Key points detection """
model = deepFinger()
model.summary()
model.load_weights('weights/deepfinger.h5')
...
for k, image_file in enumerate(image_files):
    ...
    if tl and br is not None:
        ...
        for i in range(0, len(position), 2):
            position[i] = (position[i]) * rows + tl[0]
            position[i + 1] = (position[i + 1]) * cols + tl[1]
